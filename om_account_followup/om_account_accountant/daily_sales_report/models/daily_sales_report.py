...
class DailySalesReportWizard(models.TransientModel):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', string='Branch (Analytic Account)', required=True)
    ...
    def compute_report(self):
        ...
        # search invoices of type out_invoice in date range and analytic account
        ...
        for inv in invoices:
            self.env['daily.sales.report.line'].create({
                'wizard_id': self.id,
                ...
                'paid': inv.amount_residual and (inv.amount_total - inv.amount_residual) or 0.0,
    ...
    def action_view_report(self):
        self.compute_report()
        return {
            'name': _('Daily Sales Report'),
            'type': 'ir.actions.act_window',
            'res_model': 'daily.sales.report.line',
            'view_mode': 'tree',
